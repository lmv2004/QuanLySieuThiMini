...
export const validateAccount = (form, isEdit = false) => {
    ...
    if (isEmpty(form.TENTK))
        errs.TENTK = 'Tên đăng nhập là bắt buộc.';
    else if (!isSlug(form.TENTK))
        errs.TENTK = 'Tên đăng nhập chỉ được chứa chữ cái, số, dấu chấm và gạch dưới.';
    else if (form.TENTK.length < 3)
        errs.TENTK = 'Tên đăng nhập tối thiểu 3 ký tự.';
    ...
    if (!isEdit) {
        ...
        if (isEmpty(form.MATKHAU))
            errs.MATKHAU = 'Mật khẩu là bắt buộc.';
        ...
        <div className="form-grid">
                    ...
                    <div className="form-group">
                        ...
                        <div style={{ position: 'relative' }}>
                            <input
                                ...
                                value={form.MATKHAU}
                                onChange={hc}
                                placeholder={isEdit ? '••••••• (bỏ trống = không đổi)' : '••••••• (ít nhất 6 ký tự)'}
                ...
                >
                    ...
                    {employees.map(e => (
                        <option key={e.MANV} value={e.MANV}>
                            #{e.MANV} · {e.TENNV}{e.chucVu?.TENCHUCVU ? ` — ${e.chucVu.TENCHUCVU}` : ''}{e.SODIENTHOAI ? ` (${e.SODIENTHOAI})` : ''}
                        </option>
                    ))}
                </select>
            ...
            {readOnly && (
                <div style={{ display: 'flex', gap: 12, marginTop: 4 }}>
                    <span className={form.KHOA_TK ? 'badge badge-inactive' : 'badge badge-active'}>
