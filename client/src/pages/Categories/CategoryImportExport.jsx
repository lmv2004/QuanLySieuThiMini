...
export const CategoryImportExport = ({ onRefresh, addToast, data }) => {
    ...
    const downloadTemplate = () => {
        const templateData = [{ 'Tên loại SP': 'Đồ gia dụng', 'Mô tả': 'Các sản phẩm dùng trong nhà' }];
        const ws = XLSX.utils.json_to_sheet(templateData);
        ws['!cols'] = [{ wch: 15 }, { wch: 30 }, { wch: 45 }, { wch: 20 }];
    ...
    const submitExport = () => {
        ...
        const fileName = exportName.trim() || `categories_${new Date().getTime()}`;
        const finalFileName = `${fileName}.${exportType}`;

        const exportData = data.map(v => ({'Mã loại': v.MALOAI, 'Tên loại SP': v.TENLOAI, 'Mô tả': v.MOTA || '', 'Trạng thái': v.IS_DELETED ? 'Bị khóa' : 'Hoạt động'}));
    ...
    return (
        <div className="voucher-import-export">
            ...
            {isOpen && (
                <Modal
                    title="Công cụ Nhập / Xuất Loại sản phẩm"
                    onClose={handleClose}
                    actions={
                        <>
                            ...
                            {mode === 'import' ? (
                                ...
                                <div className="ie-form-section">
                                    <div className="form-group">
                                        ...
                                        <input type="text" className="ie-input" placeholder={`categories_${new Date().getTime()}`} value={exportName} onChange={(e) => setExportName(e.target.value)} />
