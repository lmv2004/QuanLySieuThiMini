...
export const PositionImportExport = ({ onRefresh, addToast, data }) => {
    ...
    const downloadTemplate = () => {
        const templateData = [{ 'Tên chức vụ': 'Nhân viên bán hàng', 'Lương cơ bản': 5000000, 'Mô tả': 'Tư vấn và bán hàng' }];
        const ws = XLSX.utils.json_to_sheet(templateData);
        ws['!cols'] = [{ wch: 15 }, { wch: 25 }, { wch: 20 }, { wch: 40 }, { wch: 20 }];
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, 'Template');
        XLSX.writeFile(wb, 'positions_Template.xlsx');
    };
    ...
    const submitExport = () => {
        ...
        const fileName = exportName.trim() || `positions_${new Date().getTime()}`;
        const finalFileName = `${fileName}.${exportType}`;

        const exportData = data.map(v => ({'Mã chức vụ': v.MACHUCVU, 'Tên chức vụ': v.TENCHUCVU, 'Lương cơ bản': v.LUONGCOBAN || 0, 'Mô tả': v.MOTA || '', 'Trạng thái': v.IS_DELETED ? 'Bị khóa' : 'Hoạt động'}));
        ...
        ws['!cols'] = [{ wch: 15 }, { wch: 25 }, { wch: 20 }, { wch: 40 }, { wch: 20 }];
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, 'Chức vụ');
    ...
    return (
        <div className="voucher-import-export">
            ...
            {isOpen && (
                <Modal
                    title="Công cụ Nhập / Xuất Chức vụ"
                    onClose={handleClose}
                    actions={
                        <>
                            ...
                            {mode === 'import' ? (
                                ...
                                <div className="ie-form-section">
                                    <div className="form-group">
                                        ...
                                        <input type="text" className="ie-input" placeholder={`positions_${new Date().getTime()}`} value={exportName} onChange={(e) => setExportName(e.target.value)} />
